...
//      p_center.setBackground(Color.);
      ...
      t_period.setPreferredSize(new Dimension(160, 50));
...
//            exitText();
   ...
   // 이 메서드가 회원등력여부 확인 메서드 죠 ?네 
   // i will writer en english 
   public boolean check() {
      ...
      int memberID=Integer.parseInt(t_m_id.getText());
      String sql = "select * from gym_member where m_id in ('"+memberID+"')";   
      try {
         pstmt = mainPage.getCon().prepareStatement(sql);
         rs =pstmt.executeQuery(); // < --- you wrong pstmt.executeQuery 
         flag=rs.next();
      } catch (SQLException e) {
         JOptionPane.showMessageDialog(this, "아이디값이동일합니다");
         e.printStackTrace();
      }
      // Use this method !!!
      return flag=true;
